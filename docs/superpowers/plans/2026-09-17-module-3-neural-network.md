...
Expected: 7 passed. The fast config keeps this under a minute on the laptop.
Reproducibility is asserted to float32 tolerance rather than bit-exactly: `index_add_`
accumulates in thread-completion order on CPU, which moves the last bits by about
1e-8. `test_a_different_seed_gives_a_different_model` is the guard that keeps that
tolerance honest — if seeding ever breaks, that test fails even though the tolerant
one would pass.
